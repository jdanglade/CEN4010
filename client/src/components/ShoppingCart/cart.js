...
function Cart(props) {
    ...
    function handleBookDelete(book){
        API.removeFromCart(props.userEmail, book.book_id).then(res => {
            API.getCartForUser({ email: props.userEmail }).then(res => {
                setItems(res.data);
                console.log(res.data);
            })
                     ...
                     <Col md={4}>
                        <Card key = {index}>
                            ...
                            <Card.Body>
                                ...
                                <Card.Text>{books.book.quantity}</Card.Text>
                                <Button onClick={() => handleBookDelete(books.book)}>Remove From Shopping Cart</Button>
                            </Card.Body>
                        </Card>
                    </Col>
                ))}
